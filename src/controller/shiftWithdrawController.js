...
class ShiftWithdrawController {
    ...
    async store(req, res) {
        ...
        try {
            ...
            const withdraw = await ShiftWithdraw.create(req.body);

            if (!withdraw) {
                return req.status(409).json({ msg: "Error creating  cashier withdrawt" });
            }
            ...
            console.log(error);
            return res.status(400).json({ msg: 'Error creating withdraw register' });
...
module.exports = new ShiftWithdrawController();